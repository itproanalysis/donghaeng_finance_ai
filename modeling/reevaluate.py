...
def compare(before_case=BEFORE, after_case=AFTER):
    ...
    reached = MISSING
    direction = MISSING
    if is_number(result["before"]) and is_number(target):
        direction = "INCREASE" if float(target) >= float(result["before"]) else "DECREASE"
    if is_number(result["after"]) and is_number(target) and direction != MISSING:
        reached = (bool(float(result["after"]) >= float(target)) if direction == "INCREASE"
                   else bool(float(result["after"]) <= float(target)))
