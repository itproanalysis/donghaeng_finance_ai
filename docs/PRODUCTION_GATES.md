...
> 2026-09-04 범위 업데이트: 본인 Google 계정 전용 GCP 배포에 한해 외부 IdP(IAP), 영구 디스크, Secret Manager와 실제 클라우드 음성을 구현했다. [GCP 실제 서비스](GCP_LIVE_SERVICE.md)와 [점검 결과](SERVICE_AUDIT_2026-09-04.md)가 최신 구현 기록이다. 아래 광범위한 고객/기관 운영 승인 체크리스트는 여전히 별도이며 이 제한 배포로 모두 해소된 것은 아니다.
