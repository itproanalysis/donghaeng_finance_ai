...
1. `/borrower`에서 이름, 사업체명, 업종(음식점)을 입력하고 채팅 인터뷰를 시작한다.
2. AI가 매출부터 묻는다. 답변 9턴 동안 6영역 사업 지도가 채워진다.
...
6. `/interview-evaluations/{id}` 관리자 화면에 카드 세 개가 나란히 뜬다.
   공식 CB, 인터뷰 데이터 품질, 그리고 2축 스코어카드다.
7. 같은 흐름을 `?demoSet=control`로 한 번 더 돌리면 개선가능성만 30점으로 내려간
   결과를 비교할 수 있다.
...
인터뷰 화면은 이미 붙어 있다. 주소에 `?demo=`가 없으면 기존 동작 그대로이고,
있으면 대본이 순서대로 들어간다.
...
다른 화면에 붙일 때도 같은 세 값만 넘기면 된다. 답변 사이 간격은 기본 1.2초이고
...
서버가 인터뷰 결과를 위 규칙으로 옮겨 `data/live-cases/` 아래에 케이스 폴더를
만들고 `python -m modeling.scorecard`를 실행한다. 같은 규칙을 TypeScript로 다시
구현하지 않았다. 두 곳에 두면 어긋나기 때문이다.

거래 데이터는 인터뷰가 만들지 않으므로 기준 케이스에서 복사한다. 어느 케이스를
쓰는지는 화면에 그대로 적는다.
