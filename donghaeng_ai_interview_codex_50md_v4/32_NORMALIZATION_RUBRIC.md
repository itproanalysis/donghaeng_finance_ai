...
## 목적
LLM rubric classification과 deterministic normalizer를 분리한다.

## 핵심 개발 지시
- plan_specificity 0~5 rubric
- LLM은 level/reason/evidence만 반환
- normalized=level/5는 서버 계산
- 한국어 금액/비율/기간 parser
- 동일 입력 동일 결과

## 구현 메모
LLM이 normalized score를 임의 생성하지 않는다.
