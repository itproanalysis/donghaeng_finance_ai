...
## 목적
차주 발화 종료를 자동 감지해 final transcript를 만든다.

## 핵심 개발 지시
- provider endpointing 또는 server VAD
- silence threshold 700~1200ms configurable
- 답변 끝내기 버튼은 즉시 finalize
- speech_started→partial→speech_stopped→final

## 구현 메모
생각 중 짧은 침묵 때문에 지나치게 잘리지 않도록 테스트한다.
