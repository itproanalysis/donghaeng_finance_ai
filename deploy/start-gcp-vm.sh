#!/usr/bin/env bash
# Dedicated donghaeng-app VM only. Never place provider credentials in metadata.
set -euo pipefail
exec 9>/run/donghaeng-startup.lock
flock -n 9 || exit 0
...
[[ "$(metadata instance/name)" == "donghaeng-app" ]] || exit 1
...
disk=/dev/disk/by-id/google-donghaeng-data
[[ -b "$disk" ]] || exit 1
...
if [[ -z "$filesystem" ]]; then
  ...
  mkfs.ext4 -L donghaeng-data "$disk"
elif [[ "$filesystem" != "ext4" ]]; then
  echo 'Unexpected data filesystem; refusing startup.' >&2
  exit 1
fi
mountpoint_path=/mnt/disks/donghaeng-data
...
docker_config="$(mktemp -d /run/donghaeng-docker.XXXXXX)"
...
install -d -m 750 /etc/donghaeng
cat > /etc/donghaeng/runtime.env <<'ENVIRONMENT'
DONGHAENG_GCP_PROJECT_ID=abis-web-platform
DONGHAENG_AUTH_MODE=google-iap
DONGHAENG_IAP_AUDIENCE=/projects/470320899177/locations/asia-northeast3/services/donghaeng-finance-ai
DONGHAENG_IAP_ALLOWED_EMAILS=itproanalysis@gmail.com
DONGHAENG_IAP_TENANT_ID=donghaeng-gcp
DONGHAENG_APP_ORIGIN=https://donghaeng-finance-ai-jy5k5cvnjq-du.a.run.app
...
ENVIRONMENT
chmod 640 /etc/donghaeng/runtime.env

cat > /etc/systemd/system/donghaeng.service <<UNIT
[Unit]
...
TimeoutStartSec=60
TimeoutStopSec=40
ExecStart=/usr/bin/docker run --rm --name donghaeng-app --cap-drop ALL --security-opt no-new-privileges --memory 1200m --cpus 2 --log-opt max-size=10m --log-opt max-file=3 --publish 10.80.0.10:3000:3000 --env-file /etc/donghaeng/runtime.env --mount type=bind,source=$mountpoint_path/database,target=/data --mount type=bind,source=$mountpoint_path/cache,target=/app/data $image
ExecStop=/usr/bin/docker stop --time 30 donghaeng-app

[Install]
WantedBy=multi-user.target
UNIT
systemctl daemon-reload
systemctl enable donghaeng.service
systemctl restart donghaeng.service
