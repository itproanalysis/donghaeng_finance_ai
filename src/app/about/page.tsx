...
export default function AboutPage() {
  return <main id="main-content" className={styles.page}><p className={styles.eyebrow}>서비스 소개</p><h1>사업의 사정을 정리해<br />금융상담에 가져갑니다.</h1><p>동행금융은 매출 변화와 운영 계획처럼 숫자만으로 설명하기 어려운 사업 정보를 수집합니다. 답변을 원문 근거와 연결된 변수로 바꾸고, 금융기관 상담에 필요한 자료로 정리합니다.</p>
    ...
    <section id="consultation" className={styles.handoff}><h2>금융기관으로 이어지는 방법</h2><p>분석 결과나 실행 기록에서 ‘금융기관 상담 준비’를 선택합니다. 상담할 기관과 준비자료를 저장한 뒤, 원문·변수·계획·실행 기록이 담긴 준비서를 내려받습니다. 공식 안내에서 상담 절차를 확인하고 필요한 원본 자료를 함께 준비하세요.</p><div className={styles.institutionCards}>{INSTITUTION_DIRECTORY.map((item) => <article key={item.id}><small>{item.category}</small><h3>{item.name}</h3><p>{item.description}</p><a href={item.url} target="_blank" rel="noopener noreferrer">공식 상담 안내</a></article>)}</div><p className={styles.small}>위 목록은 공식 안내 경로입니다. 제휴·상품 추천·접수 완료를 뜻하지 않으며 자료가 기관으로 자동 전송되지 않습니다. 거래 은행 상담에도 준비서를 사용할 수 있습니다.</p><Link href="/review">내 결과에서 상담 준비하기</Link></section>
