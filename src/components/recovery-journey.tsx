...
export function RecoveryJourney({ interviewId, operator = false }: { interviewId: string; operator?: boolean }) {
  ...
  return <section className={styles.mission} id="recovery" aria-label={operator ? "사업자 Action과 담당자 검토" : "Recovery Journey"}>
    ...
    {state && <>
      ...
      {!state.selection && !operator && <section><h3>선택할 수 있는 개선 계획</h3><p>분석 결과와 근거를 보고 앞으로 할 일을 직접 선택하세요. 지금 선택하지 않아도 됩니다.</p><div className={styles.choices}>{dataReview?.candidates.map((candidate) => <label key={candidate.id}><input type="radio" name="recovery-action" value={candidate.id} checked={candidateId === candidate.id} onChange={(event) => setCandidateId(event.target.value)} disabled={busy} />{candidate.title}<small>{candidate.origin === "CATALOG_SUGGESTION" ? "일반 참고 제안" : "인터뷰 근거 기반 후보"} · 연결 근거 {candidate.evidenceIds.length}개</small></label>)}</div><label className={styles.consent}><input type="radio" name="recovery-action" value="SKIP" checked={candidateId === "SKIP"} onChange={() => setCandidateId("SKIP")} disabled={busy} />지금은 선택하지 않겠습니다.</label><button className={styles.primary} disabled={!candidateId || busy} onClick={() => void mutate("SELECT_ACTION", { candidateId })}>내 선택 저장</button></section>}
      {!state.selection && operator && <p>사업자가 아직 개선 계획을 선택하지 않았습니다. <Link href={`/recovery/${interviewId}`}>사업자 선택 화면</Link></p>}
      {state.selection && <div className={styles.scope}><strong>{chosen === "SKIP" ? "사업자가 선택을 보류했습니다." : chosen?.title}</strong><p className={styles.small}>선택 시각 {new Date(state.selection.selectedAt).toLocaleString("ko-KR")}<br />사업자가 직접 선택한 계획입니다. 신용판단이나 정보 품질 점수에는 사용하지 않습니다.</p></div>}
      ...
      {!operator && chosen && chosen !== "SKIP" && <form onSubmit={(event) => { event.preventDefault(); void mutate("ADD_EVIDENCE", { missionId, title, note, observedOn, expectedRevision: state.revision }); }}><h3>실행 기록 남기기</h3><p className={styles.small}>현재는 자료 준비·실행 내용을 자기보고 텍스트로 기록합니다. 파일을 업로드하거나 증빙의 진위를 확인하는 기능은 제공하지 않습니다. 실제 고객정보를 입력하지 마세요.</p><label>미션<select value={missionId} disabled={busy} onChange={(event) => { setMissionId(Number(event.target.value)); setTitle(""); setNote(""); }}>
