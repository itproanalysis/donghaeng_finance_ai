...
function ConsultationEditor({ initial, read, reviewFirst }: { initial: ConsultationPackage; read: () => Promise<ConsultationPackage>; reviewFirst: boolean }) {
  ...
  return <>
    <header><p className={styles.eyebrow}>금융기관 상담{data.environment === "SYNTHETIC_DEMO" && " · Competition Demo / Synthetic Data"}</p><h1>{reviewFirst ? "금융기관 검토자료" : <>분석 결과를<br />금융기관 상담으로 이어갑니다.</>}</h1><p>{review.businessName}의 사업 현황, 변수별 근거와 실행 기록을 금융기관 검토자료로 정리합니다.</p><nav className={styles.secondaryLinks}><Link href={`/review/${review.interviewId}`}>분석 결과</Link><Link href={`/recovery/${review.interviewId}`}>개선 계획·실행 기록</Link></nav></header>
    <ol className={styles.judgeProgress} aria-label={reviewFirst ? "금융기관 검토자료 작성 절차" : "금융기관 상담 절차"}>{(reviewFirst ? ["분석자료 확인", "담당자 의견 기록", "검토자료 저장", "금융기관 상담 준비"] : ["결과와 준비자료 확인", "상담할 기관 선택", "준비서 저장", "공식 창구에서 상담"]).map((step, index) => <li key={step}>{index + 1}. {step}</li>)}</ol>
    ...
    {reviewFirst && <>
      <section className={styles.review} aria-label="금융기관 검토용 자료"><ConsultationPrint data={data} preview /></section>
      <details className={styles.review}><summary>변수별 산식·구조화 정보·원문 추적</summary><DataReviewContent review={review} /></details>
      {data.canEditDraft && <details className={styles.review}><summary>담당자 검토 의견 작성</summary><RecoveryJourney interviewId={review.interviewId} operator onChanged={(recovery) => setData((current) => ({ ...current, recovery }))} onReviewDirtyChange={setReviewDirty} /></details>}
    </>}
    ...
    <section className={styles.handoff}><h2>준비서를 챙겨 상담 창구로</h2><p>{institution ? `${institution.name}의 공식 안내에서 상담 경로와 필요한 서류를 확인하세요.` : "준비서를 내려받아 상담할 기관의 공식 창구에서 절차를 확인하세요."}</p><div className={styles.actions}><button disabled={unavailable || persistence.dirty} onClick={() => void exportPackage("print")}><Printer size={16} />검토자료 인쇄·PDF</button><button disabled={unavailable || persistence.dirty} onClick={() => void exportPackage("json")}><Download size={16} />최종 검토자료 받기</button>{institution && <a href={institution.url} target="_blank" rel="noopener noreferrer">{institution.name} 상담 안내 <ArrowRight size={15} /></a>}</div>{reviewDirty && <p role="status">작성 중인 담당자 검토 의견을 저장한 뒤 자료를 내려받으세요.</p>}<p className={styles.small}>원문·구조화 정보·100개 변수와 근거·개선 계획·실행 기록이 자료에 포함됩니다. 기관에 자동 전송되거나 상담이 접수되지는 않습니다.</p>{notice && <p role="status">{notice}</p>}{error && <p role="alert">{error}</p>}</section>
    {!reviewFirst && <details className={styles.review}><summary>검토자료에 담기는 내용</summary><ConsultationPrint data={data} preview /></details>}
