...
/** Reused from frontend-prototype; scene checks reflect persisted mission records only. */
export default function ThreeRecoveryJourneyScene({ completedMissionIds = [] }: { completedMissionIds?: number[] }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const completedRef = useRef<readonly number[]>(completedMissionIds);
  const redrawRef = useRef<(() => void) | null>(null);
  useEffect(() => { completedRef.current = completedMissionIds; redrawRef.current?.(); }, [completedMissionIds]);
  ...
  useEffect(() => {
    ...
    catch { sceneRef.current?.classList.add("scene-unavailable"); return; }
    ...
    const interviewLabel = createLabel("분석 결과", "정보와 근거 확인 후 시작", "#17483f");
    ...
    const goalLabels = ["증빙 준비", "행동 실행", "상담 준비"];
    goalLabels.forEach((label, index) => {
      ...
      const sprite = createLabel(label, `Mission ${index + 1} · 기록 남기기`, index === 2 ? "#d49a55" : "#17483f");
    ...
    const shopLabel = createLabel("다음 금융상담", "새 Evidence를 모읍니다", "#d49a55");
    ...
    const render = () => {
      ...
      const elapsed = reduceMotion ? 3.2 : elapsedSeconds();
      ...
      goalTiles.forEach((tile, index) => {
        const checked = completedRef.current.includes(index + 1) ? 1 : 0;
      ...
      const success = completedRef.current.length === 3 ? 1 : 0;
    ...
    redrawRef.current = () => { if (reduceMotion) render(); };
    render();
  ...
  return (
    <div className="three-recovery-journey" ref={sceneRef} aria-hidden="true">
      ...
      <span className="three-scene-status">분석 이후 · 증빙 준비 → 행동 실행 → 다음 상담</span>
    </div>
  );
}
